...
- 有图文就把标题、正文、全部图片链接和 `web_url` 一次性给全
- 如果只有文字，就返回文字内容和 `web_url`
- 可以在结果后补一句很短的追问，但不能把这句追问放在结果前面
- 如果当前环境支持直接发图，先发图片，再发文字结果和 `web_url`
- 如果支持 Markdown 展示图片，优先用 Markdown 把图片展示出来，再补原始图片链接和 `web_url`
...
- 不要先问用户要不要修改，再去给结果
- 能直接发图时，先发图，再给文字结果和链接
- 能用 Markdown 展示图片时，优先用 Markdown 图片展示，不要一上来只丢链接
