...
- 有图文就把标题、正文和 `web_url` 一次性给全，并按最高优先级把图片交付出去
- 如果只有文字，就返回文字内容和 `web_url`
- 可以在结果后补一句很短的追问，但不能把这句追问放在结果前面
- 只要结果里有图片，就必须把图片发给用户，不能只给文字
- 优先级固定为：直接发图片 -> Markdown 展示图片 -> 原始图片链接
- 如果当前环境支持直接发图，先发图片，再发文字结果和 `web_url`
- 如果不能直接发图但支持 Markdown 展示图片，就用 Markdown 把图片展示出来，再发文字结果和 `web_url`
- 只有前两种都不行，才回退到原始图片链接
- `web_url` 不管什么情况都必须发
...
- 不能直发但能用 Markdown 展示图片时，优先用 Markdown 图片展示
- 只有前两种都不行，才发送原始图片链接
- `web_url` 不管什么情况都必须发
