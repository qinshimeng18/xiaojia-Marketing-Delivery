...
7. Preserve the returned `conversation_id`.
8. Run `scripts/chat_result.py --conversation-id ...` to poll the task result.
9. Read the returned JSON and use it directly:
...
12. If the user asks how to pay, buy, upgrade, open membership, or otherwise complete payment, do not construct a billing flow. Return the fixed marketing payment page from `scripts/payment_link.py` and tell the user to open that page directly.
